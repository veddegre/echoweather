...
function loadThreatLayerPrefs(){
  const saved = store.get('st_threat_layers');
  if(saved && typeof saved === 'object'){
    Object.keys(threatLayerOpts).forEach(k => {
      if(saved[k] !== undefined) threatLayerOpts[k] = !!saved[k];
    });
  }
...
async function fetchAirNow(loc){
  ...
  try{
    ...
    return { site: site + state, params, aqi: maxAqi };
  }catch(e){ return null; }
}
...
function buildAirNowDetail(airNow){
  ...
  return 'Nearest EPA monitor: ' + airNow.site + '. '
...
// ---------- master load ----------
let tabPanelsLoaded = { forecast: false, radar: false, outdoor: false, more: false };
function resetTabPanelsLoaded(){
  tabPanelsLoaded = { forecast: false, radar: false, outdoor: false, more: false };
}
function getAppTab(){
  ...
  if(document.body.classList.contains('mtab-outdoor')) return 'outdoor';
  if(document.body.classList.contains('mtab-more')) return 'more';
  return 'now';
}
function prefetchOutdoorPanels(loc, d){
  if(!loc || !d) return;
  renderExposure(d);
...
function ensureTabPanels(tab){
  ...
  if((all || tab === 'outdoor') && !tabPanelsLoaded.outdoor){
    tabPanelsLoaded.outdoor = true;
...
function scheduleIdleOutdoorPrefetch(loc){
  const run = () => {
    if(state.data && state.locations[state.active] === loc) prefetchOutdoorPanels(loc, state.data);
  };
...
async function loadAll(){
  ...
  if(fetched){
    ...
    scheduleIdleOutdoorPrefetch(loc);
  ...
  if(cached){
    ...
    prefetchOutdoorPanels(loc, cached);
    ensureTabPanels(getAppTab());
  }else{
...
// ---------- in-page nav ----------
const APP_TABS = ['now', 'forecast', 'radar', 'outdoor', 'more'];
...
function setAppTab(tab, opts){
  ...
  document.body.classList.remove('mtab-now', 'mtab-forecast', 'mtab-radar', 'mtab-outdoor', 'mtab-more');
